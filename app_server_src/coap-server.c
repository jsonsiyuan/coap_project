...
init_resources(coap_context_t *ctx) {
  ...
  coap_add_attr(r, coap_make_str_const("ct"), coap_make_str_const("0"), 0); /*text/plain; charset=utf-8*/
...
main(int argc, char **argv) 
{
  ...
  coap_startup();/*弄了一个随机送，时间，安全相关的*/
  coap_dtls_set_log_level(log_level);
  coap_set_log_level(log_level);

  printf("addr_str is %s\r\n",addr_str);
  printf("port_str is %s\r\n",port_str);
  
  ctx = get_context(addr_str, port_str);/*获得网络端口相关内容*/
  if (!ctx)
    return -1;

  init_resources(ctx);/*注册资源 time 等内容*/
